...
public class MainActivity extends Activity {
    ...
    private void showLogin(){
        base();
        root.addView(text("🐦‍🔥 FÊNIX LIVE AI",30));
        root.addView(text("Coach de performance exclusivo para usuários autorizados da Fênix.",15));
        EditText email=input("E-mail",false), pass=input("Senha",true);
        root.addView(email); root.addView(pass);
        Button login=button("Entrar"); Button recover=button("Esqueci minha senha");
        root.addView(login); root.addView(recover);
        TextView status=text("",14); root.addView(status);
        ...
        recover.setOnClickListener(v->{
            ...
            if(em.isEmpty()){ status.setText("Digite seu e-mail acima para recuperar a senha."); return; }
            status.setText("Solicitando recuperação...");
            io.execute(()->{ try{ api.resetPassword(em); runOnUiThread(()->status.setText("Se o e-mail estiver cadastrado, as instruções foram enviadas.")); }catch(Exception e){ runOnUiThread(()->status.setText("Não foi possível solicitar agora. Tente novamente.")); } });
        });
    }
    ...
    private void showDashboard(){
        ...
        root.addView(text("FÊNIX LIVE AI",29));
        ...
        Button start=button("▶ INICIAR BIGO + FÊNIX COACH");
    ...
    private void requestOverlay(){
        if(Settings.canDrawOverlays(this)){ toast("Permissão de sobreposição já está ativa."); return; }
    ...
    private void startOverlayAndBigo(){
        ...
        else toast("BIGO LIVE não encontrado neste aparelho.");
    }

    private long parseLong(String s){ try{return Long.parseLong(s.trim());}catch(Exception e){return 0;} }
    private String friendly(Exception e){ String m=e.getMessage(); if(m==null)return "Erro de conexão."; if(m.length()>160)m=m.substring(0,160); return m; }
